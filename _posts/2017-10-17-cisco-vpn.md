---
layout: post
title: 基于Cisco SSL协议的VPN解决方案
tags:
    - VPN
categories: VPN
description:  基于Cisco SSL协议的VPN解决方案
---


# 写在前面

VPN，即虚拟专用网(Virtual Private Network)，是一种常用于连接中、大型企业或团体与团体间的私人网络的通讯方法。是利用已加密的通道协议（Tunneling Protocol）来达到保密、发送端认证、消息准确性等私人消息安全效果。
...
+ 构建安全的企业内部环境，对于企业未开放至公网的网络环境提供保护。如，通过VPN访问公司内部OA系统，运维开发人员通过VPN登录公司服务器等。
+ 绕过一些特殊网络限制，访问真正的因特网。


# Cisco SSL VPN
### 服务端
OpenConnect，是一款开源的VPN软件，提供端到端的安全连接，可以在思科设备以及众多的Linux发行版进行安装和部署。
### 客户端
...
思科SSL协议也是基于普通SSL协议，故而需要域名和证书来提供服务，证书的获取方式可以通过自签名证书或者购买证书服务服务商提供的证书。众多的证书服务商提供了免费的DEV证书可供申请，可以免费获得。
修改必要的参数配置，最终得到的配置如下：
...
ping-leases = false
route = 10.200.0.0/255.255.0.0  # 需要路由的IP或者IP段
...
route = 8.8.8.8/0 # usera需要路由的IP或者IP段
...
```
rx-data-per-sec = 20000000
tx-data-per-sec = 20000000
```
